...
class CalcForgeEngine:
    ...
    def evaluate_expression(self, expr, sheet_id=0, line_num=1):
        ...
        try:
            ...
            # PERFORMANCE: Fast early detection of incomplete expressions to prevent 300ms eval() delays
    ...
    def _is_incomplete_expression(self, expr):
        """
        PERFORMANCE: Fast detection of incomplete expressions to prevent slow eval() calls.
        Returns True if the expression is incomplete and should not be evaluated.
        """
        ...
        # Check for incomplete function calls (ends with open parenthesis)
        if expr.strip().endswith('('):
            return True

        # Check for incomplete operators (ends with operator)
        if expr.strip() and expr.strip()[-1] in '+-*/^=<>':
            return True

        # Check for incomplete decimal numbers (ends with decimal point)
        if expr.strip().endswith('.') and not expr.strip().endswith('..'):
            return True
        ...
        # Check for function names without parentheses (likely incomplete typing)
        # Common function patterns that are likely incomplete
        incomplete_patterns = [
            r'\b(sin|cos|tan|log|ln|sqrt|abs|round|floor|ceil|max|min|sum|mean|tc|ar|truncate|tr)$',
            r'\b[a-zA-Z]+\($',  # Any word followed by open paren at end
        ]
