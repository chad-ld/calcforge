...
class SelectiveCalculationRequest(BaseModel):
    expressions: List[str]
    line_numbers: List[int]  # Which lines to actually calculate
    sheet_id: int = 0


class SelectiveCalculationResponse(BaseModel):
    results: Dict[int, CalculationResponse]  # line_number -> result
...
@app.post("/api/calculate-selective", response_model=SelectiveCalculationResponse)
async def calculate_selective(request: SelectiveCalculationRequest):
    """
    Calculate only specific lines for incremental updates.
    This is much faster than batch calculation for small changes.
    """
    print(f"DEBUG: calculate_selective called with {len(request.line_numbers)} lines")
    results = {}

    # Only process the specified line numbers
    for line_num in request.line_numbers:
        if 1 <= line_num <= len(request.expressions):
            expression = request.expressions[line_num - 1]  # Convert to 0-based index

            try:
                # Skip empty lines and comments
                if not expression.strip() or expression.strip().startswith(':::'):
                    results[line_num] = CalculationResponse(
                        value="",
                        unit="",
                        error=None,
                        highlights=[]
    ...
    return SelectiveCalculationResponse(results=results)
