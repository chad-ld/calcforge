...
class TabManager {
    ...
    createTab(name = null, content = '') {
        ...
        const tabData = {
            ...
            // PERFORMANCE: Initialize cache properties for fast tab switching
            cachedResults: null,
            lastCalculatedContent: null
        };
    ...
    switchToTab(tabId) {
        ...
        // Save current tab content AND calculation results
        if (this.activeTabId && this.editor) {
            const currentTab = this.tabs.get(this.activeTabId);
            if (currentTab) {
                currentTab.content = this.editor.getText();
                currentTab.lastModified = new Date();
                // PERFORMANCE: Cache calculation results for fast tab switching
                currentTab.cachedResults = this.editor.getLastResults();
                currentTab.lastCalculatedContent = this.editor.getLastCalculatedContent();
            }
        }
        ...
        // PERFORMANCE OPTIMIZATION: Use cached results if available, otherwise calculate
        if (this.editor && newTab.content.trim()) {
            if (newTab.cachedResults && newTab.lastCalculatedContent === newTab.content) {
                // Use cached results for instant tab switching
                console.log(`PERFORMANCE: Using cached results for tab ${tabId} (${newTab.cachedResults.length} results)`);
                this.editor.setLastResults(newTab.cachedResults, newTab.lastCalculatedContent);
                this.editor.updateResults(newTab.cachedResults);
            } else {
                // Content changed or no cache, need to recalculate
                console.log(`PERFORMANCE: Recalculating tab ${tabId} (cache miss or content changed)`);
